...
def get_data(num_train_examples, num_test_examples, batch_size, random_labels, binary_digits, mnist_first_class, mnist_second_class):
    if binary_digits:
        train_superset = []
        for data in trainset:
            if data[1] == class_to_index_dict["ship"] or data[1] == class_to_index_dict["dog"]:
...
def normalize_data_even_odd(data, target):
    ...
    # dog = 5 vs ship = 8
    target = torch.Tensor([1 if i == 5 else -1 for i in target]).type(torch.LongTensor)
