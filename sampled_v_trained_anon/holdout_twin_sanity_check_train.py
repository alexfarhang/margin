...
import json
from util_full_train.trainer import train_network
...
from util_full_train.trainer import AdaptiveConvNet, ConvNet, SimpleNet
from util_full_train.nero import Nero,neuron_mean,neuron_norm
...
flags.DEFINE_integer('batch_size', 200, 'batch size to use while training')
...
def main(argv):
    ...
    if dataset_type == "mnist":
        ...
        if network_arch == "conv" or network_arch == 'conv_adaptive':
            ...
            from util_full_train.cifar10_data import get_data, normalize_data_even_odd
        else:
    ...
    if dataset_type == "mnist":
        ...
        print("Invalid dataset_type... returning...")
    ...
    train_acc_list, test_acc_list, model, train_loss_list, test_loss_list, train_total, test_total = train_network(
    ...
    #         train_data, train_target = (train_data.cuda(), train_target.cuda())
    #         y_pred = model_trained(train_data).squeeze()
    #         loss = (y_pred - train_target).norm()
    ...
    #         data, target = (data.cuda(), target.cuda())
    #         y_pred = model_trained(data).squeeze()

    #         total_train_loss += (y_pred - target).norm()
    #         train_correct += (target.float() == y_pred.sign()).sum().item()
    ...
    #         data, target = (data.cuda(), target.cuda())
    #         y_pred = model_trained(data).squeeze()
    #         total_test_loss += (y_pred - target).norm()
    #         test_correct += (target.float() == y_pred.sign()).sum().item()
    ...
    #     test_data, test_target = normalize_data_even_odd(test_data, test_target)
        ...
        #             'error_trained': (model_trained(train_data).squeeze() - y_pred).norm().item(),
